...
- **Social share cards**: `scripts/generate-og-images.mjs` writes a 1200×630 JPEG card for each post's image into `public/resources/og/` (gitignored, regenerated by `build:full`). Wide images are cover-cropped; portrait, square, and small images sit whole on a blurred copy of themselves. Prerender uses the card for `og:image` / `twitter:image` and falls back to the original image if a card is missing.
...
| `npm test` | Run Vitest unit tests once (230 tests) |
...
| `npm run build:full` | Typecheck + mirror assets + snapshot READMEs + thumbnails + share cards + Vite + SSR bundle + prerender (use before deploy) |
...
| `npm run generate:og` | Write 1200×630 social share cards into `public/resources/og/` |
...
| `shared/resourceOgImages.test.ts`, `scripts/generate-og-images.test.mjs` | Social share card path mapping and generation |
